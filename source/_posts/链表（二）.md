...
![](/image/链表(二)/链表(二)-6.png)
2.  当小孩报数时，让first 和 helper 指针同时 的移动  m  - 1 次
![](/image/链表(二)/链表(二)-7.png)
